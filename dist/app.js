// @ts-nocheck
import { getOverlayUrl, buildTeamPayload, fetchPokemonLocalized, fetchFrenchPokemonIndex, fetchPokemonLanguages, fetchPokemonFormSuggestions, saveTeam, loadTeam, getProfile, updateProfileLanguage, DEFAULT_OVERLAY_STYLE, shouldUsePixelRendering, getNextEvolutionName } from "./shared.js";
...
function setPaldeaManualBadges(value = "") {
    ...
    PALDEA_BADGE_KEYS.forEach((badge, index) => {
        if (paldeaBadgeSliders[index])
            paldeaBadgeSliders[index].checked = selected.has(badge);
    });
}
...
function refreshSlotTranslations() {
    for (let index = 0; index < 6; index++) {
        ...
        if (title)
            title.textContent = t("app.slot_title", { index: index + 1 });
        ...
        const levelInput = wrapper.querySelector(`#level${index}`);
        if (levelInput)
            levelInput.placeholder = t("app.level");
        const itemInput = wrapper.querySelector(`#item${index}`);
        if (itemInput)
            itemInput.placeholder = t("app.item");
        const evolutionBtn = wrapper.querySelector(`#evolutionBtn${index}`);
        if (evolutionBtn)
            evolutionBtn.textContent = t("app.evolution");
        const swapLabel = wrapper.querySelector(`label[for="swapTarget${index}"]`);
        if (swapLabel)
            swapLabel.textContent = t("app.swap_with_slot");
        const swapBtn = wrapper.querySelector(`#swapBtn${index}`);
        if (swapBtn)
            swapBtn.textContent = t("app.swap");
        const deadBtn = wrapper.querySelector(`#deadBtn${index}`);
        if (deadBtn)
            deadBtn.textContent = t("app.dead");
    }
}
...
async function saveCurrentTeam() {
    ...
    try {
        ...
        if (autosaveIndicator)
            autosaveIndicator.textContent = t("common.status.saving");
        await saveTeam(channel, payload);
        setStatus(t("app.status.team_saved"), "success");
        if (autosaveIndicator)
            autosaveIndicator.textContent = t("app.synced");
        updateOverlayLink();
    }
    catch (error) {
        console.error(error);
...
function queueAutoSave() {
    if (!autoSaveInput?.checked)
        return;
    if (autosaveIndicator)
        autosaveIndicator.textContent = t("app.queued");
...
async function init() {
    ...
    const persistLanguage = async (event) => {
        try {
            await updateProfileLanguage(session.channel, session.editKeyHash, sanitizeLanguage(event.detail?.language || getCurrentLanguage()), { preHashed: true });
        }
        catch (error) {
    ...
    syncNuzlockeUi();
    if (spriteScaleValue)
        spriteScaleValue.textContent = `${Math.max(50, Math.min(1000, Number(spriteScale.value) || 100))}%`;
    syncShinyAvailability();
    renderEditorCanvas();
