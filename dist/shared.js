...
async function migrateLegacyEditKey(channel, profile) {
    if (!profile?.editKey || profile.editKeyHash)
        return;
...
async function fetchSpeciesLocalizedNames(speciesUrl) {
    ...
    const localizedNames = Object.fromEntries((data.names || [])
        ...
        return Promise.all(results.map(async (entry) => {
            try {
                ...
                if (!detailsResponse.ok)
                    throw new Error("Language details unavailable");
...
function toApiCandidate(value, index = []) {
    ...
    const match = index.find(entry => normalizeInput(entry.frenchName) === normalized ||
...
export async function fetchPokemonLocalized(inputName, shiny = false, spriteOptions = {}) {
    ...
    const artworkDefault = pokemonData?.sprites?.other?.["official-artwork"]?.front_default ||
        pokemonData?.sprites?.other?.home?.front_default ||
        pokemonData?.sprites?.front_default ||
        "";
    const artworkShiny = pokemonData?.sprites?.other?.["official-artwork"]?.front_shiny ||
...
export function buildTeamPayload({ trainerName, badgeText, nuzlockeMode, deathCount, slots, displayOptions, overlayStyle }) {
    ...
    return {
        ...
        displayOptions: {
            ...
            badgeCount: Math.max(0, Math.min(18, Number(displayOptions.badgeCount) || 0)),
            badgeManual: cleanText(displayOptions.badgeManual),
...
export async function generateShareCode(channel, editKeyOrHash, { autoRotateOnUse = false, expiresInHours = 24, preHashed = false } = {}) {
    const normalized = normalizeChannel(channel);
    const profile = await getProfile(normalized);
