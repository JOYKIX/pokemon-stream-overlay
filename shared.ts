...
function getLocalizedFormLabel(formData = {}, preferredLanguages = ["fr", "en"]) {
  ...
  for (const language of requestedLanguages) {
    ...
    if (fromFormNames) return fromFormNames;
...
function buildLocalizedFormLabels(formDetail = {}, fallback = "") {
  ...
  const addLabel = (language, value) => {
    ...
    if (!key || !label || labels[key]) return;
    labels[key] = label;
  };
  ...
  if (englishLabel) labels.en = labels.en || englishLabel;
  if (frenchLabel) labels.fr = labels.fr || frenchLabel;
...
async function fetchPokemonFormMetadata(inputName, index = []) {
  ...
  const promise = (async () => {
    ...
    const options = await Promise.all(varieties.map(async (variety) => {
      ...
      if (!pokemonName || pokemonName === baseName) return null;
...
function resolveFormSpecificPokemonName(form = "", metadata = null) {
  ...
  if (!normalized || !metadata?.options?.length) return "";
  const tokens = normalized.split(" ").filter(Boolean);

  const exact = metadata.options.find((option) => option.aliases.includes(normalized));
  if (exact?.pokemonName) return exact.pokemonName;
...
export async function fetchPokemonFormSuggestions(inputName, language = "en") {
  ...
  if (!metadata?.options?.length) return [];
  const normalizedLanguage = cleanText(language);
  const labelLanguage = normalizedLanguage || "en";
  return uniqueValues(metadata.options.map((option) => (
    option.localizedLabels?.[labelLanguage] ||
